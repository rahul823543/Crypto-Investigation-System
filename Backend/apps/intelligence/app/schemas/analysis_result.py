...
──────────────────────────────
Pydantic v2 response DTO for POST /v1/analyze per BACKEND_PLAN_v3 §3, §7, §8.
...
Fastify validates the following fields after Python responds:
  ...
  - vaspAttribution conforms to VaspAttribution schema or is null
...
class VaspAttribution(BaseModel):
    """
    Nearest-VASP attribution result produced by attribution/vasp_attribution.py.
    Reuses the same traversal as risk detection, filtered to vasp-labeled nodes.
    'nearest' = highest surviving confidence when a vasp-labeled node is reached.

    Set to None (not omitted) when no confident VASP match is found before the
    traversal stopping condition triggers. Fastify validates path IDs exist
    in the submitted graph before persisting.
    """

    model_config = {"populate_by_name": True}

    attributed_vasp: str = Field(
        alias="attributedVasp",
        description="Human-readable VASP name e.g. 'Binance'",
    )
    vasp_node_id: str = Field(
        alias="vaspNodeId",
        description="Stable node ID of the attributed VASP node",
    )
    hop_distance: int = Field(alias="hopDistance", ge=1)
    confidence: float = Field(
        ge=0.0,
        le=1.0,
        description="confidence_at_hop(hop_distance_to_vasp_node) per v3 §7",
    )
    path_node_ids: list[str] = Field(
        alias="pathNodeIds",
        description="Ordered node IDs from root to VASP node",
    )
    path_edge_ids: list[str] = Field(
        alias="pathEdgeIds",
        description="Ordered edge IDs from root to VASP node",
    )
    basis: str = Field(
        description="Human-readable explanation e.g. 'connected via 2 hops to labeled Binance deposit'"
    )
    secondary_candidates: list[dict] = Field(
        alias="secondaryCandidates",
        default_factory=list,
        description="Other reachable VASP nodes with lower confidence",
    )
...
class AnalysisResponse(BaseModel):
    ...
    # v3 §7: vaspAttribution is null (not omitted) when no confident match is found.
    vasp_attribution: VaspAttribution | None = Field(
        alias="vaspAttribution",
        default=None,
        description="null when no confident VASP match found before stopping condition",
    )
